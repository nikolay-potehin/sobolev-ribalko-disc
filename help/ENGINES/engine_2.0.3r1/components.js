		...
		listeners:{
			select :function(combo, record, index){
				...
				var items =root.eachChild(function(child){childs.push(child)});
					...
					root.appendChild(Ext.sp.Tree.get(record.data.name));	
...
Ext.extend(Ext.ux.Book, Ext.Panel, {  	  
					...
					if(pre.leaf){
						if(pre.sco){
							...
							Ext.Ajax.request({url: path, success: function(r){
								...
								if(i == self._items.length-1) self.initBlocks();
							   }
							})	
						}
	...
	filter:function(blocktypeid){		
		...
		this.body.query('div[id=block]').each(function(block){
			var  show = pressed.some(function(button){return button.id == block.className});
...
function $bookmarks(){
				...
				buttons: [{				
					...
					handler:function(){
						...
						if(text == ""){
							...
							data.bookmarks.include([id,block>=0?block:null,text]);							
	...
	bookmarkgrid.getSelectionModel().on('rowselect', function(sm, rowIdx, r) {		
		$show(r.data.id,false,r.data.block);		
	});
...
var Notes = {
	...
	showNote:function(id, block){		
		var note = this.get(id, block);			
		var htmlEditor = new Ext.form.HtmlEditor({value:note?note[2]:""});
